...
const MyAnnouncements = () => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const response = await fetch("https://volunteershub-6.onrender.com/api/announcements");
        if (!response.ok) {
  ...
  const handleDeleteAnnouncement = async (id) => {
    try {
      const response = await fetch(`https://volunteershub-6.onrender.com/api/announcements/${id}`, {
        method: "DELETE",
      });
